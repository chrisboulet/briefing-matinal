...
- **Modèle** : **`grok-4.5`** par défaut (ID live confirmé `GET /v1/models` 2026-07-17 ; pin Hermes `BRIEFING_XAI_MODEL`). Fallbacks possibles : `grok-4.3`, puis `grok-4.20-0309-reasoning` si qualité insuffisante. **Aucun alias `-latest`** côté xAI — les modèles sont versionnés explicitement. Les anciens IDs `grok-4-1-fast-*` ne sont plus exposés.
