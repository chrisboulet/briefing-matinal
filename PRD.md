...
- **Modèle** : **`grok-4-1-fast-non-reasoning`** par défaut (optimisé agentic tool calling, pricing bas ~0.20 $/M input, 0.50 $/M output, pas de chain-of-thought — le sourcing est une tâche d'extraction + ranking structuré). Escalade en 2 temps si qualité insuffisante après 2 semaines : (1) `grok-4-1-fast-reasoning` (mêmes tokens input + thinking tokens, gain qualité marginal), puis (2) `grok-4.20-0309-reasoning` (flagship complet, coût 10× plus élevé). **Aucun alias `-latest`** côté xAI (confirmé `GET /v1/models`) — les modèles sont versionnés explicitement.
